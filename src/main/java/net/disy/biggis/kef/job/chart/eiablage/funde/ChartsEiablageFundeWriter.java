...
@Component
@StepScope
public class ChartsEiablageFundeWriter implements ItemWriter<ChartDto> {
  ...
  public void write(List<? extends ChartDto> data) throws Exception {
    for (ChartDto current : data) {
      fileService.writeChartsEiablageFunde(current.getId(), current.getYear(), current.getContent());
    }
