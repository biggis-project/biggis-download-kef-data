...
@Component
@StepScope
public class ChartsEiablageBeerenWriter implements ItemWriter<ChartDto> {
  ...
  public void write(List<? extends ChartDto> data) throws Exception {
    for (ChartDto current : data) {
      fileService.writeChartsEiablageBeeren(current.getId(), current.getYear(), current.getContent());
    }
